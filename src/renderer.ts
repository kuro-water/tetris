...
const main = async function () {
    const {
        ...
        SRS_I,
        SRS_TLJSZ,
    } = await electronAPI.readJson(MINO_DATA_PATH);
    // このへんpromise.thenでやろうとしたけどconstだから次に引き継がれないのよね
    // なんか良い書き方ないかなあ
    ...
    class Mino {
         ...
         * @param context
         */
        drawMino(x = this.x, y = this.y, color = MINO_COLORS[this.idxMino], context = this.contextField) {
            ...
            for (let i = 1; ; i++) {
                for (const block of this.blocks) {
                    if (this.field.isFilled(this.x + block.x, this.y + block.y + i)) {
                        // ゴーストの描画
                        this.drawMino(this.x, this.y + i - 1, GHOST_COLORS[this.idxMino], this.contextField);
        ...
        checkRotation(dif: number, rotated: number[][], move: number[]): boolean {
            ...
            for (let i = 0; i < 4; i++) {
                ...
                for (let j = 0; j < 4; j++) {
                    // 移動先の検証
                    if (this.field.isFilled(this.x + rotated[0][j] + move[0], this.y + rotated[1][j] + move[1])) {
                        // console.log("braek:" + i);
    ...
    class Tetris {
        ...
        clearFieldContext() {
            ...
            this.contextField.fillRect(FIELD_CANVAS_SIZE[2] - BLOCK_SIZE, 0, BLOCK_SIZE, FIELD_CANVAS_SIZE[3]);
            this.contextField.fillRect(0, FIELD_CANVAS_SIZE[3] - BLOCK_SIZE, FIELD_CANVAS_SIZE[2], BLOCK_SIZE);
        }
